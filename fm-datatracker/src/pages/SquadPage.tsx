...
export function SquadPage(){
  ...
  return <div className="screen-page squad-page">
    ...
    <div className="table-wrap squad-table customizable-squad-table"><table><colgroup>{columns.map(column=><col style={{width:widths[column.id]??defaultWidth(column)}} key={column.id}/>)}</colgroup><thead><tr>{columns.map((column,index)=><SortHeader key={column.id} label={column.label} column={column.id} sort={sort} change={changeSort} frozen={index<=frozenIndex} frozenEdge={index===frozenIndex} left={frozenLeft(columns,index,widths)} draggable dragStart={()=>setDragColumn(index)} dragOver={event=>event.preventDefault()} drop={()=>dragColumn!==null&&moveColumn(dragColumn,index)} resize={event=>resizeColumn(index,event.clientX)} context={event=>{event.preventDefault();setColumnMenu({x:event.clientX,y:event.clientY,index})}} description={column.key==='reference'?'Usamos o maior percentil entre todas as linhas em que o jogador atua.':undefined}/>)}</tr></thead><tbody>{rows.map(row=><tr key={row.player.id} onClick={()=>navigate(`/players/${row.player.id}`)} className="clickable">{columns.map((column,index)=><SquadCell key={column.id} column={column} row={row} referenceCountry={referenceCountry} referenceDivision={referenceDivision} model={model} frozen={index<=frozenIndex} frozenEdge={index===frozenIndex} left={frozenLeft(columns,index,widths)}/>)}</tr>)}</tbody></table></div>
...
function SquadCell({column,row,referenceCountry,referenceDivision,model,frozen,frozenEdge,left}:{column:TableColumn;row:Row;referenceCountry:string;referenceDivision:number;model:ModelConfig;frozen:boolean;frozenEdge:boolean;left:number}){
  ...
  if(key==='name')return <td className={`frozen-player-name ${cellClass}`} style={style}><strong>{row.player.current_name}</strong></td>
