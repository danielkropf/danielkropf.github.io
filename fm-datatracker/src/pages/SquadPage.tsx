...
const columnLabels:Record<ColumnKey,string>={status:'Status',name:'Nome',age:'Idade',nationality:'Nacionalidade',value:'Valor',team:'Equipe',position:'Posições',score:'Nota',reference:'Nível de referência'}
...
export function SquadPage(){
  ...
  const[positionFilters,setPositionFilters]=useState<string[]>([]),[columns,setColumns]=useState<ColumnKey[]>(()=>{try{const saved=JSON.parse(localStorage.getItem('fm-datatracker:squad-columns')??'null');return Array.isArray(saved)?allColumns.filter(column=>saved.includes(column)):allColumns}catch{return allColumns}})

  useEffect(()=>{void loadReferenceDataset().then(setReference)},[])
  useEffect(()=>{localStorage.setItem('fm-datatracker:squad-columns',JSON.stringify(columns))},[columns])
  ...
  }).filter(row=>filters.every(filter=>matchesFilter(row,filter))&&(!positionFilters.length||positionFilters.some(target=>canPlayPosition(row.latest?.positions??[],target)))).sort((a,b)=>compareRows(a,b,sort.key)*sort.direction||a.player.current_name.localeCompare(b.player.current_name,'pt-BR')),[players,search,sort,activeWeights,evaluation,position,referenceScores,model.planning,filters,positionFilters])
  ...
  return <div className="screen-page squad-page">
    ...
    <section className="squad-evaluation-bar"><label className="compact-control">Avaliação<select value={evaluation} onChange={event=>setEvaluation(event.target.value as 'general'|'role')}><option value="general">Melhor referência</option><option value="role">Por função</option></select></label>{evaluation==='role'&&<><div className="phase-compact"><button className={phase==='IP'?'active':''} onClick={()=>setPhase('IP')}>IP</button><button className={phase==='OOP'?'active':''} onClick={()=>setPhase('OOP')}>OOP</button></div><select value={position} onChange={event=>setPosition(event.target.value)}>{positions.map(([value,label])=><option value={value} key={value}>{label} · {value}</option>)}</select><select value={selectedRole[0]} onChange={event=>setRoleCode(event.target.value)}>{roleOptions.map(([code,name])=><option value={code} key={code}>{code} · {name}</option>)}</select></>}<PositionChooser selected={positionFilters} change={setPositionFilters}/><ColumnChooser columns={columns} change={setColumns}/><button className={`filter-toggle ${filters.length?'active':''}`} onClick={()=>setFilterOpen(true)}>Filtros {filters.length?`(${filters.length})`:''}</button></section>
    <div className="table-wrap squad-table"><table><thead><tr>{columns.map(column=><SortHeader key={column} label={column==='score'?(evaluation==='general'?'Nota geral':roleName):columnLabels[column]} column={column} sort={sort} change={changeSort} description={column==='reference'?'No modo Melhor referência, usamos o maior percentil entre todas as linhas em que o jogador atua. No modo Por função, todos são comparados na posição e função selecionadas.':undefined}/>)}</tr></thead><tbody>{rows.map(row=><tr key={row.player.id} onClick={()=>navigate(`/players/${row.player.id}`)} className={`clickable ${row.compatible?'':'position-incompatible'}`}>{columns.map(column=><SquadCell key={column} column={column} row={row} referenceCountry={referenceCountry} referenceDivision={referenceDivision}/>)}</tr>)}</tbody></table></div>
...
function ColumnChooser({columns,change}:{columns:ColumnKey[];change:(value:ColumnKey[])=>void}){return <details className="position-multi-filter column-chooser"><summary>Colunas <span>{columns.length}/{allColumns.length}</span><i>⌄</i></summary><div className="position-multi-menu"><header><b>Colunas visíveis</b><button type="button" onClick={()=>change(allColumns)}>Todas</button></header>{allColumns.map(column=><label key={column}><input type="checkbox" checked={columns.includes(column)} disabled={column==='name'} onChange={event=>change(event.target.checked?[...columns,column]:columns.filter(item=>item!==column))}/><span>{columnLabels[column]}</span></label>)}</div></details>}
function SquadCell({column,row,referenceCountry,referenceDivision}:{column:ColumnKey;row:Row;referenceCountry:string;referenceDivision:number}){if(column==='status')return <td><span className={`planning-status ${row.status==='Não selecionado'?'unselected':'selected'}`}>{row.status}</span></td>;if(column==='name')return <td><strong>{row.player.current_name}</strong></td>;if(column==='age')return <td>{row.latest?.age??'—'}</td>;if(column==='nationality')return <td>{row.player.nationality||'—'}</td>;if(column==='value')return <td>{row.marketValue||'—'}</td>;if(column==='team')return <td>{row.latest?.club||row.latest?.squad||'—'}</td>;if(column==='position')return <td>{row.latest?.positions?.join(', ')||'—'}</td>;if(column==='score')return <td><ScoreBadge value={row.score} rank={row.referencePercentile} className="score-badge-compact"/></td>;return <td>{row.referencePercentile===null?'—':<Tooltip content={`P${row.referencePercentile}: nota igual ou superior à de ${row.referencePercentile}% dos ${row.referenceSample} jogadores aptos em ${row.referenceGroup}, na ${referenceDivision}ª divisão de ${referenceCountry}.${row.compatible?'':' Este jogador não possui a posição selecionada nos dados importados.'}`}><span className={`reference-level level-${row.referenceLevel?.toLowerCase().replaceAll(' ','-')}`} tabIndex={0}><b>P{row.referencePercentile}</b> {row.referenceLevel} · {row.referenceGroup}</span></Tooltip>}</td>}
