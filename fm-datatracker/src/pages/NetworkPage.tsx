...
export function NetworkPage() {
  ...
  useEffect(() => {
    ...
    void Promise.all([loadCurrentPlayers(selected.id), loadModelConfig(selected.id)])
      .then(([rows, model]) => { if (active) { setPlayers(rows as Player[]); setConfig(model as NetworkConfig) } })
  ...
  const source = useMemo(() => players.map(player => {
    ...
    const owner = typeof normalized.contract_current_team_name === 'string' ? normalized.contract_current_team_name : typeof normalized.contract_team_name === 'string' ? normalized.contract_team_name : snapshot?.club ?? null
    return { player, snapshot, plannedId, owner, team: snapshot?.squad ?? 'Não resolvida', conflicts: index.conflicts[player.id] ?? [] }
  }), [players, index])
  const pool = useMemo(() => source.filter(row => (currentFilter === 'all' || row.snapshot?.club === currentFilter)
  ...
  const currentClubs = useMemo(() => [...new Set(source.map(row => row.snapshot?.club).filter((value): value is string => Boolean(value)))].sort(), [source])
  ...
  return <div className="roadmap-page">
    ...
    <section className="card network-pool">
      ...
      <div className="network-table" role="table"><div className="network-row network-head" role="row"><span>Jogador</span><span>Atual / proprietário</span><span>Destino / equipe</span><span>Situação</span></div>{pool.map(({ player, snapshot, plannedId, owner, team, conflicts }) => <div className="network-row" role="row" key={player.id}><Link to={`/players/${player.id}`}>{player.current_name}</Link><span>{snapshot?.club ?? 'Não resolvido'}<small>{owner && owner !== snapshot?.club ? `Prop.: ${owner}` : 'Vínculo atual'}</small></span><span>{plannedId ? clubNames.get(plannedId) ?? 'Clube não acompanhado' : 'Sem destino'}<small>{team}</small></span><span>{conflicts.length ? <b className="negative">Conflito em {conflicts.length} clubes</b> : plannedId ? 'Planejado' : 'Disponível'}</span></div>)}</div>
    </section>
  </div>
}
