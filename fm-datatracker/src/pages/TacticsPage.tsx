...
export function TacticsPage(){
  ...
  return <div className="tactics-page">
    ...
    <div className="tactic-toolbar"><select aria-label="Tática selecionada" value={tactic?.id??''} onChange={e=>setConfig(c=>({...c,selected_tactic_id:e.target.value||null,selected_role_id:null}))}><option value="">Selecione uma tática</option>{config.tactics.map(t=><option value={t.id} key={t.id}>{t.name}</option>)}</select><button onClick={()=>setCreateOpen(true)}>+ Adicionar</button><button className="danger-button" disabled={!tactic} onClick={remove}>Excluir</button></div>
