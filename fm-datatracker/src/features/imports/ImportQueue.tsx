...
type Job = { id: string; save: Save; name: string; file?: File; updateTarget?: ImportUpdateTarget; phase: ImportProgress['phase'] | 'done'; detail: string; progress?: number; confirmation: number }
...
export function ImportQueueProvider({ children }: { children: ReactNode }) {
  ...
  function add(files: File[] = [], updateTarget?: ImportUpdateTarget) {
    ...
    setJobs(old => [...old, { id, save: selected, name: updateTarget?.original_filename ?? 'CSV ou importação combinada', file: files[0], updateTarget, phase: 'idle', detail: 'Selecione os arquivos para leitura.', confirmation: 0 }])
    setActive(id)
  }
  ...
  function completed(id: string, summary: string) {
    ...
    setJobs(old => old.map(j => j.id === id ? { ...j, phase: 'done', detail: summary, progress: 100, file: undefined } : j))
...
export function ImportQueueIndicator() {
  ...
  const progress = aggregateImportProgress(jobs)
  return <ImportProgressRing value={progress.value} complete={progress.complete} label={progress.complete ? 'Todas as tarefas concluídas' : `${progress.pending} tarefas restantes`} center={progress.pending} />
}
...
export function ImportQueueLauncher() {
  ...
  return <section className="import-center"><header><h1>Importar arquivos</h1><p>Destino dos novos arquivos: <strong>{selected?.name ?? 'Selecione um save'}</strong></p></header>
    {queue.jobs.length > 0 && <div className="import-overall-progress"><ImportQueueIndicator /><div><strong>Progresso geral · {aggregateImportProgress(queue.jobs).value}%</strong><small>{queue.jobs.filter(j => j.phase === 'done').length} de {queue.jobs.length} tarefas salvas · progresso por etapas</small></div></div>}
