...
type VersionedImportRecord = ImportRecord & ImportUpdateTarget
...
export function ImportsPage({ mode = 'import' }: ImportsPageProps) {
  ...
  const updateTarget = useRef<VersionedImportRecord | null>(null)
  ...
  return <div className="screen-page imports-page">{mode !== 'history' && <ImportQueueLauncher />}<section className="settings-import-history import-inline-history"><input hidden type="file" ref={updateInput} aria-label="Arquivo original para atualização" onChange={event=>{const files=Array.from(event.target.files??[]);const target=updateTarget.current;event.target.value='';if(!target||!files.length)return;const combined=sourceLabel(target.original_filename)==='CSV + .fm';if(files.filter(f=>/\.fm$/i.test(f.name)).length!==1 || (combined && files.filter(f=>/\.csv$/i.test(f.name)).length!==1) || files.length!==(combined?2:1)){setMessage(combined?'Selecione o .fm e o CSV originais juntos.':'Selecione somente o .fm original.');return}importQueue.add(files,target)}} />
