...
export function unassignedSquadPlayerIds(players: Array<{ id: string; squadId: string | null; observed: boolean }>, groupId: string, sets: Record<string, string[]>): string[] {
  const assigned = new Set(Object.values(sets).flat())
  return players.filter(player => player.observed && player.squadId === groupId && !assigned.has(player.id)).map(player => player.id)
}
export type UnassignedDrawerState = { search: string; position: string; selected: string[]; anchor: string | null; target: string; scrollTop: number }
