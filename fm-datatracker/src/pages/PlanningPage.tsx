import { LeagueComparisonHint, useLeagueReference, referenceTeamForSave } from '../components/LeagueComparisonHint'
import type { LeagueReference, LeagueRolePair } from '../lib/league-reference'
...
import { countryFlagEmoji, currentRosterStatus, type CurrentRosterMembershipKind, type PlanningTeamLevel } from '../lib/current-roster'
...
import { PLANNING_PITCH_LIST_CAPACITY, planningPitchPositionLabel, planningPitchSetHeader, planningPitchVisibleSlotCount } from '../lib/planning-pitch-list'
...
export function PlanningPage({ active = true }: PlanningPageProps = {}) {
  ...
  const [drawerRequestedSet, setDrawerRequestedSet] = useState<{id:string;revision:number}>()
  ...
  const leagueState = useLeagueReference(selected?.id, currentCheckpoint?.saveId === selected?.id && currentCheckpoint?.status === 'ready' ? currentCheckpoint.date : null, currentCheckpoint?.revision ?? 0)
  const leagueTeam = referenceTeamForSave(leagueState.data)
  ...
  const factualSquadForPlayer = (playerId: string) => {
    const fact = membershipFact(playerId)
    if (fact.kind === 'loaned_out' || fact.kind === 'other_club') return null
    return fact.membership?.squad_name?.trim() || latestByPlayer.get(playerId)?.squad?.trim() || null
  }
  const teamLevelForPlayer = (playerId: string): PlanningTeamLevel => (membershipFact(playerId).membership?.team_level as PlanningTeamLevel | undefined) ?? null
  const squadGroupForPlayer = (playerId: string) => effectivePlanningSquadGroupId(planning, playerId, factualSquadForPlayer(playerId), teamLevelForPlayer(playerId))
  const marketGroupForPlayer = (playerId: string) => planning.groups.find(group => isMarketPlanningGroup(group) && Object.values(planning.slotAssignments[group.id] ?? {}).some(ids => ids.includes(playerId))) ?? null
  const rosterStatusForPlayer = (playerId: string) => { const fact = membershipFact(playerId); return hasPlayerMarketFlag(planning, playerId, 'loan') && hasPlayerMarketFlag(planning, playerId, 'sale') ? 'Para venda e empréstimo' : currentRosterStatus(fact.kind === 'other_club', marketGroupForPlayer(playerId), fact.kind as CurrentRosterMembershipKind) }
  ...
  useEffect(() => {
    ...
    setConfig(current => ({ ...current, ...patchClubPlanning(current, selectedClubId, primaryClubId, next) }))
  }, [selectedClubId, planning, players, membershipFacts, primaryClubId])
  ...
  const unassignedItems = useMemo<UnassignedDrawerItem[]>(() => {
    ...
    return players.filter(player => ids.has(player.id)).map(player => {
      ...
      const setRatings = currentSets.map(set => {
        ...
        return { id: set.id, label: setHeaderLabel(set).split('\n')[0], eligible: slots.some(candidate => candidate.familiar), value: chosen?.score ?? null, position: chosen?.position ?? null, score: renderRating(chosen) }
      })
      return { peek: <PlayerPeek player={player} snapshot={snapshot} />, bestSetLabel: currentSets.find(set=>best && set.slotIds.includes(best.slotId)) ? setHeaderLabel(currentSets.find(set=>best && set.slotIds.includes(best.slotId))!).split('\n')[0] : null, setRatings, id: player.id, name: player.current_name, age: snapshot.age, positions: snapshot.positions, bestPosition: best?.position ?? null, bestRole: best ? `${best.pair.ip.position} · ${best.pair.ip.roleName} / ${best.pair.oop.position} · ${best.pair.oop.roleName}` : null, eligiblePositions: [...new Set(candidates.filter(c=>c.familiar).map(c=>c.position))], hasEligible: candidates.some(c=>c.familiar), value: best?.score ?? null, status: rosterStatusForPlayer(player.id), score: renderRating(best) }
  ...
  function placePlayer(groupId: string, setId: string, playerId: string, beforePlayerId?: string | null) {
    const currentSquad = squadGroupForPlayer(playerId)
    let target = movePlayerToSet(planning, groupId, setId, playerId, beforePlayerId) as Planning
    if (isMarketPlanningGroup(planning.groups.find(group => group.id === groupId))) { if (currentSquad) target = movePlayerToPlanningSquad(target, playerId, currentSquad) as Planning }
    else target = movePlayerToPlanningSquad(target, playerId, groupId) as Planning
    if (!selectedClubId) { update(() => target); return }
    setUndoPlanning({ planning: config.planning, planning_by_club: config.planning_by_club })
  ...
  function movePlayerToSquad(playerId: string, groupId: string) {
    const target = movePlayerToPlanningSquad(planning, playerId, groupId) as Planning
    if (!selectedClubId) { update(() => target); return }
    setUndoPlanning({ planning: config.planning, planning_by_club: config.planning_by_club })
    setConfig(current => { const planningByClub = movePlayerAcrossClubPlans(current.planning_by_club ?? {}, selectedClubId, playerId, target); return { ...current, planning_by_club: planningByClub, ...(selectedClubId === primaryClubId ? { planning: planningByClub[selectedClubId] } : {}) } })
  }
  ...
  const pickerRowsBase = useMemo<PickerRow[]>(() => {
    ...
    return players.flatMap(player => {
      const snapshot = latestByPlayer.get(player.id)
      const playerSquadGroupId = squadGroupForPlayer(player.id)
      if (!pickerShowOtherSquads && playerSquadGroupId !== currentGroup.id) return []
  ...
  }, [pickerSet, currentGroup, planning, currentSets, players, latestByPlayer, playerScores, referenceRatings, membershipFacts, planningIndex, planningClubs, potential.showPotential, pickerShowPotential, pickerShowOtherSquads, potential.ceilingModel, potential.ceilingModel?.manifest.potentialModelVersion])
  ...
  function renderPickerCell(row: PickerRow, column: PickerColumn): ReactNode {
    ...
    return row.snapshot ? <ScoreWithProjection playerId={row.player.id} currentScore={row.score} currentRank={row.rank} rankPopulation={row.rankPopulation} snapshot={row.snapshot} scoreType="function" scoreKey={row.projectionKey} variant="compact" currentTitle="Nota atual nesta função" showPotential={potential.showPotential && pickerShowPotential} /> : '—'
  }
  ...
  return <div className="screen-page planning-page planning-flex-page planning-pitch-list-page">
    ...
    <section className={`planning-depth-layout planning-flex-layout planning-full-pitch-layout ${!isTransferGroup ? 'has-unassigned-rail' : ''} ${drawerOpen && !isTransferGroup ? 'has-unassigned-drawer' : ''}`}>
      <div className={`planning-flex-board ${expandedSets.size ? 'has-expanded' : ''} ${isTransferGroup ? 'is-transfer' : ''}`}>
        {!isTransferGroup && currentSets.length > 0 && <button type="button" className="ghost planning-expand-all" aria-label={currentSets.every(set=>expandedSets.has(set.id)) ? 'Recolher todos' : 'Expandir todos'} title={currentSets.every(set=>expandedSets.has(set.id)) ? 'Recolher todos' : 'Expandir todos'} onClick={()=>setExpandedSets(currentSets.every(set=>expandedSets.has(set.id)) ? new Set() : new Set(currentSets.map(set=>set.id)))}><span aria-hidden="true">{currentSets.every(set=>expandedSets.has(set.id)) ? '⊟' : '⊞'}</span></button>}
        {isTransferGroup && currentGroup ? <TransferGroupPanel group={currentGroup} playerIds={planning.slotAssignments[currentGroup.id]?.market ?? []} players={players} latest={latest} fact={membershipFact} plannedClub={plannedClubName} dragging={Boolean(activePlayer)} drop={() => { if (activePlayer) placePlayer(currentGroup.id, 'market', activePlayer.id); stopPlayerDrag() }} startDrag={id => setDragging({ type: 'player', id })} dragEnd={stopPlayerDrag} open={id => navigate(`/players/${id}`)} context={openPlayerMenu} remove={playerId => update(value => togglePlayerMarketFlag(value, playerId, currentGroup.id as 'loan' | 'sale') as Planning)} />
          : tactic && currentGroup ? <div className={`planning-set-list is-spatial-ready ${expandedSets.size ? 'has-expanded' : ''}`}>{currentSets.map(set => <PlanningSetRow leagueData={leagueState.data} leagueTeam={leagueTeam} leagueMessage={leagueState.message} leagueRequestedDate={leagueState.requestedDate} leagueSourceDate={leagueState.sourceDate} leagueFallback={leagueState.fallback} leaguePairs={setPairs(set).map(pair => ({ label: `${pair.ip.position} ${pair.ip.roleName} / ${pair.oop.position} ${pair.oop.roleName}`, ip: { position: pair.ip.position, weights: resolvedWeights(pair.ip, 'IP') }, oop: { position: pair.oop.position, weights: resolvedWeights(pair.oop, 'OOP') } }))} key={set.id} set={set} spatial={spatialPlacements.get(set.id)} displayLabel={displaySetLabel(set)} headerLabel={setHeaderLabel(set)} pairs={setPairs(set)} assignedIds={planning.slotAssignments[currentGroup.id]?.[set.id] ?? []} players={players} latest={latest} expanded={expandedSets.has(set.id)} focused={focusedSetId === set.id} coverages={showCoverages ? coveragePlayers(set) : []} showCoverages={showCoverages} showScores={showScores} generalScore={generalScore} scoreDetails={tacticScoreDetails} primaryLabel={primaryLabel} activePlayer={activePlayer} playerDropPreview={playerDropPreview} score={player => setScore(player, set)} familiarity={player => setFamiliarity(player, set)} fact={membershipFact} plannedClub={plannedClubName} plannedConflict={plannedClubConflict} rosterStatus={rosterStatusForPlayer} toggle={() => toggleSet(set.id)} focus={() => { setFocusedSetId(set.id); setDrawerOpen(true); setDrawerRequestedSet(old=>({id:set.id,revision:(old?.revision ?? 0)+1})) }} addPlayer={() => { setPickerSearch(''); setPickerSelected(new Set()); setPickerAnchor(null); setPickerSetId(set.id) }} startPlayerDrag={(id, event) => { event.dataTransfer.effectAllowed = 'move'; event.dataTransfer.setData('text/plain', id); setDragging({ type: 'player', id }); setPlayerDropPreview(null) }} stopPlayerDrag={stopPlayerDrag} previewPlayer={beforePlayerId => setPlayerDropPreview({ setId: set.id, beforePlayerId })} dropPlayer={beforePlayerId => { if (activePlayer) placePlayer(currentGroup.id, set.id, activePlayer.id, beforePlayerId); stopPlayerDrag() }} open={id => navigate(`/players/${id}`)} context={openPlayerMenu} moveVisualGrid={cell => moveSetVisualPosition(set.id, cell)} />)}</div>
      ...
      {drawerOpen && !isTransferGroup && currentGroup && <PlanningUnassignedDrawer requestedSet={drawerRequestedSet} key={`${selectedClubId}:${currentGroup.id}:${tactic?.id}`} items={unassignedItems} sets={currentSets.map(set=>({id:set.id,label:setHeaderLabel(set).split('\n')[0]}))} initialState={drawerRestore?.groupId===currentGroup.id?drawerRestore.state:undefined} onProfile={openUnassignedProfile} onContext={openPlayerMenu} onDragStart={id=>setDragging({type:'player',id})} onDragEnd={stopPlayerDrag} onAssign={assignUnassigned} />}
    </section>
...
function PlanningSetRow({ leagueData, leagueTeam, leagueMessage, leagueRequestedDate, leagueSourceDate, leagueFallback, leaguePairs, set, spatial, displayLabel, headerLabel, pairs, assignedIds, players, latest, expanded, focused, coverages, showCoverages, showScores, generalScore, scoreDetails, primaryLabel, activePlayer, playerDropPreview, score, familiarity, fact, plannedClub, plannedConflict, rosterStatus, toggle, focus, addPlayer, startPlayerDrag, stopPlayerDrag, previewPlayer, dropPlayer, open, context, moveVisualGrid }: {
  leagueData: LeagueReference | null; leagueTeam: number | null; leagueMessage: string; leagueRequestedDate: string | null; leagueSourceDate: string | null; leagueFallback: boolean; leaguePairs: LeagueRolePair[];
  set: PlanningSetLayout; spatial: PlanningGridPlacement | undefined; displayLabel: string; headerLabel: string; pairs: Pair[]; assignedIds: string[]; players: Player[]; latest: (player: Player) => Snapshot | undefined; expanded: boolean; focused: boolean; coverages: Player[]; showCoverages: boolean; showScores: boolean; generalScore: (player: Player) => number | null; scoreDetails: (player: Player) => PlanningScoreDetail[]; primaryLabel: (playerId: string) => string; activePlayer?: Player; playerDropPreview: PlayerDropPreview | null; score: (player: Player) => { pair: Pair | null; value: number | null; rank: number | null; rankPopulation: number[] }; familiarity: (player: Player) => Familiarity; fact: (playerId: string) => PlanningMembershipFact; plannedClub: (playerId: string) => string | null; plannedConflict: (playerId: string) => string[]; rosterStatus: (playerId: string) => string; toggle: () => void; focus: () => void; addPlayer: () => void; startPlayerDrag: (id: string, event: DragEvent<HTMLElement>) => void; stopPlayerDrag: () => void; previewPlayer: (beforePlayerId: string | null) => void; dropPlayer: (beforePlayerId?: string | null) => void; open: (id: string) => void; context: (event: ReactMouseEvent, playerId: string) => void; moveVisualGrid: (cell: PlanningVisualGridCell) => void
}) {
  ...
  return <article ref={articleRef} data-spatial-key={spatial?.key ?? set.id} data-spatial-side={spatial?.side ?? 'center'} data-grid-row={spatial?.gridRow} data-grid-column={spatial?.gridColumn} data-set-label={displayLabel} data-grid-locked={spatial?.isGoalkeeper ? 'goalkeeper' : undefined} style={spatialStyle} className={`planning-set-row planning-line-${planningLine(linePosition)} ${grouped ? 'is-grouped' : ''} ${expanded ? 'is-expanded' : ''} ${focused ? 'is-focused' : ''} ${visualGridPreview !== null ? 'is-visual-position-dragging' : ''} ${preview !== undefined && activePlayer ? 'is-player-drop-target' : ''}`} onPointerDown={startVisualDrag} onPointerMove={moveVisualDrag} onPointerUp={finishVisualDrag} onPointerCancel={cancelVisualDrag} onLostPointerCapture={event => { if (visualDragRef.current?.pointerId === event.pointerId) cancelVisualDrag(event) }} onDragOver={event => { if (activePlayer) { event.preventDefault(); previewPlayer(null) } }} onDrop={event => { if (!activePlayer) return; event.preventDefault(); dropPlayer(preview ?? null) }}>
    {visualGridOverlay}
    <LeagueComparisonHint data={leagueData} team={leagueTeam} pairs={leaguePairs} message={leagueMessage} requestedDate={leagueRequestedDate} sourceDate={leagueSourceDate} fallback={leagueFallback}>
      <button type="button" className="planning-set-league-comparison" aria-label={`Informações de ligas de ${displayLabel}`} onPointerDown={event => event.stopPropagation()} onClick={event => event.stopPropagation()}>i</button>
    ...
    {expanded && <button className="planning-set-collapse" onClick={event => { event.stopPropagation(); toggleExpansion() }}>−</button>}
  </article>
}
...
function BoardPlayerRow({ player, snapshot, score, generalScore, scoreDetails, showScores, rank, rankPopulation, coverage, source, familiarity, fact, plannedClub, plannedConflict, status, projectionKey, familiarityTooltip, dragging, drag, dragEnd, open, context }: { player: Player; snapshot: Snapshot | undefined; score: number | null; generalScore: number | null; scoreDetails: PlanningScoreDetail[]; showScores: boolean; rank: number | null; rankPopulation: number[]; coverage: boolean; source: string | null; familiarity: Familiarity; fact: PlanningMembershipFact; plannedClub: string | null; plannedConflict: string[]; status: string; projectionKey: string; familiarityTooltip: string; dragging: boolean; drag: (event: DragEvent<HTMLElement>) => void; dragEnd: () => void; open: () => void; context: (event: ReactMouseEvent) => void }) {
  ...
  return <article data-planning-player-id={player.id} className={`planning-pitch-depth-row ${coverage ? 'is-coverage' : ''} ${out ? 'is-out-of-position' : ''} ${!snapshot ? 'is-current-unknown' : ''} ${!showScores ? 'is-score-hidden' : ''} ${dragging ? 'is-player-dragging' : ''} ${statusPlayerClass(status)}`} draggable onDragStart={event => { event.stopPropagation(); drag(event) }} onDragEnd={dragEnd} onContextMenu={context}>
    <PlanningPitchRowCells
      showScores={showScores}
      peek={snapshot ? <PlayerPeek player={player} snapshot={snapshot} /> : null}
      identity={<div className={`planning-pitch-player-name ${statusPlayerClass(status)}`}><button type="button" className="planning-player-name-link" onClick={event => { event.stopPropagation(); open() }}>{player.current_name}</button><small className="planning-player-meta">{snapshot ? `${snapshot.age ?? '—'} anos` : 'Sem observação atual'}{countryFlagEmoji(player.nationality) ? <> · <span className="planning-country-flag" aria-label={player.nationality ?? undefined}>{countryFlagEmoji(player.nationality)}</span></> : ''}</small></div>}
      score={scoreContent}
    />
  </article>
}
