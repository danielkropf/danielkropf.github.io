...
describe('FM26 offline result normalizer', () => {
  it('keeps every decoded stat and maps the validated player fields without runtime data', () => {
    ...
    expect(result.players[0].normalized_data).toMatchObject({ source: 'fm26-save-offline', ca_pa_status: 'candidate_with_provenance_not_universally_validated', left_foot: 14, right_foot: 20 })
    ...
    expect(result.players[0].normalized_data).toMatchObject({
      contract_team_name: 'FC Bayern München', contract_team_semantics_status: 'owner_unresolved_person_team_relation',
      roster_team_name: 'FC Bayern München II',
    })
  })
