...
function normalizedVersion(value: unknown) { return typeof value === 'string' && /^\d{12}$/.test(value) ? value : null }
function capability(source: Record<string, unknown>, key: string) { return source[key] === true }
function shouldReuseCache(force: boolean) { if (force || !cached) return false; if (cachedStatus === 'compatible') return true; return Date.now() - cachedAt < STALE_RECHECK_MS }

export function parseDatabaseCompatibilityInfo(value: unknown): DatabaseCompatibility {
  if (!isRecord(value)) return { status: 'unversioned', schemaVersion: null, capabilities: { ...NO_CAPABILITIES }, diagnostic: 'O banco respondeu sem metadata de schema reconhecível.' }
  const schemaVersion = normalizedVersion(value.schema_version)
  if (!schemaVersion) return { status: 'unversioned', schemaVersion: null, capabilities: { ...NO_CAPABILITIES }, diagnostic: 'O banco respondeu sem uma versão de schema reconhecível.' }
  if (schemaVersion < REQUIRED_DATABASE_SCHEMA) return { status: 'outdated', schemaVersion, capabilities: { ...NO_CAPABILITIES }, diagnostic: `Schema ${schemaVersion}; esta versão espera ${REQUIRED_DATABASE_SCHEMA}.` }
  if (!isRecord(value.capabilities)) return { status: 'unversioned', schemaVersion, capabilities: { ...NO_CAPABILITIES }, diagnostic: 'O schema informa uma versão recente, mas não expõe capability detection explícita. Atualize as migrations antes de continuar.' }
  ...
  const capabilities: DatabaseCapabilities = {
    importRpc: capability(source, 'import_rpc'), deleteImportRpc: capability(source, 'delete_import_rpc'), modelConfigPatch: capability(source, 'model_config_patch'), projections: capability(source, 'projections'),
...
export function checkDatabaseCompatibility(force = false): Promise<DatabaseCompatibility> {
  ...
  cached = (async () => {
    ...
    if (error) {
      ...
      if (isMissingRpcError(error) || info.code === 'PGRST202') return { status: 'unversioned', schemaVersion: null, capabilities: { ...NO_CAPABILITIES }, diagnostic: 'O banco online não possui o marcador de schema desta versão.' } as DatabaseCompatibility
  ...
  })().then(result => { cachedStatus = result.status; return result })
  return cached
}

export function resetDatabaseCompatibilityCache() { cached = null; cachedAt = 0; cachedStatus = null }
