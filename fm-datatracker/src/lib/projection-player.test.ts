...
describe('projection player facts', () => {
  it('usa GK e OUTFIELD como famílias gerais', () => {
  ...
  it('extrai CP e personalidade sem expor CA', () => {
    const facts = snapshotProjectionFacts({ snapshot_date: '2026-08-26', positions: ['M (C)'], raw_data: { birth_date: '2008-01-01' }, normalized_data: { fm_hidden: { potential_ability: 166, professionalism: 18, ambition: 14 } }, player_attributes: [{ attribute_key: 'determination', value: 16 }] })
    expect(facts.cp).toBe(166)
  ...
  it('usa baseline neutro quando a personalidade escondida não existe', () => {
    const facts = snapshotProjectionFacts({ snapshot_date: '2026-08-26', positions: ['M (C)'], raw_data: { birth_date: '2008-01-01' }, normalized_data: { potential_ability: 150 }, player_attributes: [] })
    expect(facts.cp).toBe(150)
