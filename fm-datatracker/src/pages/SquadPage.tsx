...
export function SquadPage(){
  ...
  return <div className="screen-page squad-page">
    <div className="title-row"><div><h1>{selected?.club_name}</h1>{(loading||isPending)&&<span className="background-loading" role="status">Atualizando elenco em segundo plano…</span>}</div><div className="squad-actions"><label>Referência<select value={referenceCountry} onChange={event=>setReferenceCountry(event.target.value)}>{referenceCountries.map(country=><option value={country} key={country}>{country}</option>)}</select></label><label>Divisão<select value={referenceDivision} onChange={event=>setReferenceDivision(Number(event.target.value))}>{referenceDivisions.map(division=><option value={division} key={division}>{division}ª divisão</option>)}</select></label><input className="search" placeholder="Buscar jogador" value={search} onChange={event=>setSearch(event.target.value)}/></div></div>
