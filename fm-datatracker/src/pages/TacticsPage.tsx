...
export function TacticsPage(){
  ...
  function swapPhaseLinks(targetPhase:TacticPhase,targetPlayerId:string){
    if(!tactic||!draggingCard||draggingCard.phase!==targetPhase||draggingCard.playerId===targetPlayerId)return
  ...
  const pickerRows=pickerPair?rankedCandidates(pickerPair.ip,pickerPair.oop).filter(({candidate})=>[candidate.name,candidate.positions.join(' '),candidate.age].join(' ').toLowerCase().includes(playerPickerSearch.toLowerCase())).map(row=>({...row,generalScore:attributeScore(row.candidate.attributes.map(attribute=>({key:attribute.key,value:attribute.value,weight:config.general_weights[attribute.key]??3})))})):[]
