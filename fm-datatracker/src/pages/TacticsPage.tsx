...
import{canonicalRoleDefaultWeights,pairedRoleScore,resolveRoleWeights}from'../lib/role-scoring'
import{ScoreWithProjection}from'../components/ScoreWithProjection'
import{functionProjectionKey,type ProjectionSnapshot}from'../lib/projection-player'
...
type Candidate={id:string;name:string;positions:string[];age:number|null;attributes:Array<{key:string;value:number}>;snapshot:ProjectionSnapshot|null}
...
function normalizeTactic(t:Partial<Tactic>&Pick<Tactic,'id'|'name'>):Tactic{
  ...
  for(const a of [...ipAssignments,...oopAssignments]){
    ...
    if(existing){existing.name=a.roleName;if(usesLegacyRoleDefaults(existing.weights))existing.weights=canonicalRoleDefaultWeights(a.roleId,a.roleName)}
    else roles.push({id:a.roleId,name:a.roleName,weights:canonicalRoleDefaultWeights(a.roleId,a.roleName)})
  }
...
export function TacticsPage(){
  ...
  useEffect(()=>{
    ...
    void loadCurrentPlayers(selected.id).then(rows=>{
      ...
      setCandidates(rows.map(player=>{
        ...
        return{id:player.id,name:player.current_name,positions:snapshot?.positions??[],age:snapshot?.age??null,attributes:(snapshot?.player_attributes??[]).map(a=>({key:a.attribute_key,value:a.value})),snapshot:snapshot?{id:snapshot.id,snapshot_date:snapshot.snapshot_date,age:snapshot.age,positions:snapshot.positions,raw_data:snapshot.raw_data,normalized_data:snapshot.normalized_data,player_attributes:snapshot.player_attributes.map(a=>({attribute_key:a.attribute_key,value:a.value}))}:null}
      }))
  ...
  function changeAssignmentRole(slotId:string,targetPhase:TacticPhase,roleCode:string){
    ...
    setConfig(c=>({...c,tactics:c.tactics.map(t=>{
      ...
      const roles=t.roles.some(r=>r.id===roleId)?t.roles:[...t.roles,{id:roleId,name:option[1],weights:canonicalRoleDefaultWeights(roleId,option[1])}]
  ...
  function scoreFor(candidate:Candidate,ip:Assignment,oop:Assignment){
    ...
    const weights=(assignment:Assignment)=>resolveRoleWeights({roleId:assignment.roleId,roleName:assignment.roleName,overrideWeights:config.role_weight_overrides[assignment.roleId]??tactic.roles.find(role=>role.id===assignment.roleId)?.weights})
