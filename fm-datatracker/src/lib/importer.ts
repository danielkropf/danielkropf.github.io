...
import { ATTRIBUTE_CATALOG, ATTRIBUTE_LOOKUP } from './attributes'
...
/** Parses the file in Papa Parse's worker so the app remains responsive while it is being read. */
...
export async function filesHash(files:File[]){const fingerprints=await Promise.all(files.map(async file=>digestHex(await crypto.subtle.digest('SHA-256',await file.arrayBuffer()))));fingerprints.sort();return fileHash(fingerprints.join(':'))}
export function isValidIsoDate(value:string){if(!/^\d{4}-\d{2}-\d{2}$/.test(value))return false;const[y,m,d]=value.split('-').map(Number);const date=new Date(Date.UTC(y,m-1,d));return date.getUTCFullYear()===y&&date.getUTCMonth()===m-1&&date.getUTCDate()===d}
