...
function SettingsModal({close}:{close:()=>void}){const[tab,setTab]=useState<'saves'|'imports'|'quality'|'resources'>('saves');return <div className="settings-overlay" onClick={close}><section className="settings-modal" onClick={event=>event.stopPropagation()}><header><div><span className="eyebrow">ADMINISTRAÇÃO</span><h1>Configurações</h1></div><button className="close" onClick={close}>×</button></header><div className="settings-tabs"><button className={tab==='saves'?'active':''} onClick={()=>setTab('saves')}>Saves</button><button className={tab==='imports'?'active':''} onClick={()=>setTab('imports')}>Imports</button><button className={tab==='quality'?'active':''} onClick={()=>setTab('quality')}>Qualidade dos dados</button><button className={tab==='resources'?'active':''} onClick={()=>setTab('resources')}>Downloads</button></div><div className="settings-content">{tab==='saves'?<SavesPage/>:tab==='imports'?<Protected><ImportsPage/></Protected>:tab==='quality'?<Protected><QualityPage/></Protected>:<Resources/>}</div></section></div>}
