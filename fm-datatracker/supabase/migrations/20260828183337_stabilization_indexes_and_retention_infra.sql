-- FM DataTracker stabilization: restore a historical lookup index that is present in the
-- canonical migration chain, cover the diagnostic composite FK, and provision the
-- database extensions used by the server-side retention scheduler.
...
create index if not exists fm_reader_samples_save_owner_idx
  on public.fm_reader_samples(save_id, owner_id);
