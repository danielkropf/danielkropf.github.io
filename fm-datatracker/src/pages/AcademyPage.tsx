...
export function AcademyPage() {
  ...
  return <div className="roadmap-page">
    ...
    {selected && <AutomaticIntakes key={selected.id} saveId={selected.id} currentPlayers={currentPlayers}/> }
    {error && <p className="roadmap-alert" role="alert">{error}</p>}
    <section className="roadmap-kpis"><Metric value={filtered.length} label="turmas CSV/manuais"/><Metric value={members.length} label="jogadores registrados"/><Metric value={observed} label="observados no checkpoint"/><Metric value={currentKnown} label="clube atual confirmado"/>{unknown > 0 && <Metric value={unknown} label="sem observação atual"/>}</section>
    ...
    <section className="academy-grid">
      ...
      {!loading && !filtered.length && <article className="card roadmap-empty"><h2>Nenhuma turma CSV/manual neste recorte</h2><p>As turmas identificadas no .fm aparecem na seção própria. Fatos históricos também podem ser registrados abaixo.</p></article>}
    </section>
